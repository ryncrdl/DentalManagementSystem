Imports System.Reflection.Emit

Public Class deletedrejected
    Private d As New d()
    Private Sub SwitchTable1(table As UserControl)
        'Form.Size = New Size(945, 640)
        table.Dock = DockStyle.Fill
        table.BackColor = Color.White
        PanelTable.Controls.Clear()
        PanelTable.Controls.Add(table)
    End Sub

    Private Sub BtnIneligible_Click(sender As Object, e As EventArgs) Handles BtnIneligible.Click
        Me.Close()
    End Sub

    Private Sub deletedrejected_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        SwitchTable1(d)
    End Sub
End Class